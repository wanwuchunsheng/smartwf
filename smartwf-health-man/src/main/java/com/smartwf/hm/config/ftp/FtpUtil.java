...
public class FtpUtil {
	...
	public static boolean ftpUpload(String fileName, String ftpUrl, int ftpPort,
		...
		try {
			...
			if (isConnection) {
				boolean isSuccess = ftp.upload(fileName,ftpRemotePath, ftpLocalDir );
	 ...
	 *             异常
	 */
	public boolean upload(String fileName,String remotePath, String localPath) throws IOException {
		// 进入被动模式
		mFTPClient.enterLocalPassiveMode();
		// 以二进制进行传输数据
		mFTPClient.setFileType(FTP.BINARY_FILE_TYPE);
